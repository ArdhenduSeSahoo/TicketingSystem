...
import { AllStringConditions } from "@/lib/DefaultData/FilterData/ConditionDataAll";
import FilterStringConditionDdwItem from "./FilterStringConditionDdwItem";
...
export interface IFilterStringConditionDdw {
  filterUIData: FilterUIData;
}

export default function FilterStringConditionDdw(
  props: IFilterStringConditionDdw,
) {
  ...
  //if (props.filterUIData.filterType === 1)
  {
    ...
    AllStringConditions.forEach((itm, index) => {
  ...
  function onChangeFire(e) {
    for (const key in AllStringConditions) {
      if (AllStringConditions[key].name === e.target.value) {
        dispatch(
          changeFilterCondition({
            filterRow: props.filterUIData,
            newFilterCondition: AllStringConditions[key],
          }),
        );
      }
    }
