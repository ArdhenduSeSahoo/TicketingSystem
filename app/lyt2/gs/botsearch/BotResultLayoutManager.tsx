...
import { useAppDispatchBotSearch, useAppSelectorBotSelector } from '@/lib/Redux/Hooks/BotSearchHooks';
import { fetchBotGlobalSearchData } from '@/lib/Redux/Slices/BotSearchSlices/BotListDataSlice';
import { useEffect } from 'react';



export default function BotResultLayoutManager () {
    const dispatch=useAppDispatchBotSearch();
    const selectbotdata=useAppSelectorBotSelector(selectBotResultData);
    useEffect(()=>{
     if (selectbotdata.botTopIntent.includes("show all")&&selectbotdata.botShowAllQuery!== "" &&selectbotdata.hasError == false) {
       ...
       //console.log("Calling list data ui");
     }
    //  console.log("bot layout--"+selectbotdata.errorMessage);
    //  console.log("bot layout--" + selectbotdata.hasError);
    //  else if(selectbotdata.)
    //  else{
